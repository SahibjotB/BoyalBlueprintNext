...
function SubmitButton() {
    ...
    return (
        ...
        >
            {pending ? <Loader2 className="mr-2 h-5 w-5 animate-spin" aria-hidden="true" /> : "Sign In"}
        </button>
    )
}

export function LoginForm() {
    const [state, formAction] = useActionState(loginUser, { error: "", success: false })

    return (
        <form action={formAction} className="w-full space-y-5" aria-label="Sign In Form">
