...
export function buildODataQuery(filters: Record<string, any>) {
    ...
    for (const [key, value] of Object.entries(filters)) {
        ...
        if (key === "BedroomsTotal" && typeof value === "number") {
            // get the variations of bedroom with helper function
            const variants = generateBedroomFallback(value);
            ...
            odataQueryString.push(`(BedroomsTotal eq ${value} or ${fallbackQuery.join(" or ")})`);
            continue;
        } 

        if (key === "BathroomsTotalInteger" && typeof value === "number") {
            // get the variations of washroom with helper function
            const variants = generateWashroomFallback(value);

            const fallbackQuery = variants.map(
                variant => `contains(PublicRemarks, '${variant}')`
            );
            odataQueryString.push(`(BathroomsTotalInteger eq ${value} or ${fallbackQuery.join(" or ")})`);
            continue;
        } 

        if (key === "BuildingAreaTotal" && typeof value === "number") {
            // get the variations of sqft
            const variants = generateSqftFallback(value);

            const fallbackQuery = variants.map(
                variant => `contains(PublicRemarks, '${variant}')`
            );
            odataQueryString.push(`( BuildingAreaTotal eq ${value} or ${fallbackQuery.join(" or ")})`);
            continue;
        } 
